...
capture log close
log using "$log/SEN_PMT_Model_Performance_Rural.log", replace 
...
twoway kdensity lpcexp_pred [aw=$weight] || kdensity lpcexp_pred if b40==1 [aw=$weight], legend(order(1 "All Rural HBS" 2 "Rural HBS in Bottom 40%") size(small)) title("Distribution of predicted log expenditure (PMT scores)")
gr save "$results/HBS_pmtscore_distribution_rural.gph",  replace
...
foreach truepoor in poor_actual_20 poor_actual_37 {
	...
	save "$results/SEN2021_SWIFTPMT_targeting_rural_small_performance_`truepoor'.dta",replace
...
*** Undercoverage and Leakage barcharts for rural models ***
use "$results/SEN2021_SWIFTPMT_targeting_rural_small_performance_poor_actual_20.dta", clear
gen model = "Smaller"
append using "$results/SEN2021_SWIFTPMT_targeting_rural_large_performance_poor_actual_20.dta"
...
graph bar under_s under_l, ///
    ...
    title("Rural Models: Undercoverage by Coverage Rate and Model", size(medlarge)) ///
	subtitle("Target Population: Bottom 20%") ///
    ytitle("Undercoverage") ///
    ylabel(0(.1).6) ///
    legend(order(1 "Smaller Model" 2 "Larger Model"))	
	
gr save "$results/SEN2021_SWIFTPMT_rural_undercoverage_models.gph",replace
...
graph bar leak_s leak_l, ///
    over(cutoff, label(angle(0))) ///
    title("Rural Models: Leakage by Coverage Rate and Model", size(medlarge)) ///
	subtitle("Target Population: Bottom 20%") ///
    ytitle("Leakage") ///
    ylabel(0(.1).6) ///
    legend(order(1 "Smaller Model" 2 "Larger Model"))	
	
gr save "$results/SEN2021_SWIFTPMT_rural_leakage_models.gph",replace
