...
capture log close
log using "$log/SEN_PMT_Model_Performance_Urban_fixedpr.log", replace 
...
twoway kdensity lpcexp_pred [aw=$weight] || kdensity lpcexp_pred if b40==1 [aw=$weight], legend(order(1 "All Urban HBS" 2 "Urban HBS in Bottom 40%") size(small)) title("Urban HBS: Distribution of Predicted Log Expenditure (PMT scores)", size(medsmall)) 
