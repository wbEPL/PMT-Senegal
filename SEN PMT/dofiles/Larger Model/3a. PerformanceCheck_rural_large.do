...
* Performance Checks -- RURAL ONLY							   *
* Jeremy Schneider 2025								   *
****************************************************************
...
capture log close
log using "$log/SEN_PMT_Performance_largermodel_rural.log", replace 
...
twoway kdensity lpcexp_pred [aw=$weight] || kdensity lpcexp_pred if b40==1 [aw=$weight], legend(order(1 "All Rural HBS" 2 "Rural HBS in Bottom 40%") size(small)) title("Rural HBS (Larger Model): Distribution of Predicted Log Expenditure (PMT scores)", size(small)) 
...
foreach truepoor in poor_actual_20 poor_actual_37 {
	...
	save "$results/SEN2021_SWIFTPMT_targeting_rural_large_performance_`truepoor'.dta",replace
...
/*
	preserve 
	keep if _n == 1 
	save "$results/SEN2021_SWIFTPMT_targeting_rural_large.dta",replace
	restore 
...
save "$results/SEN2021_SWIFTPMT_leakage_rural_large.dta", replace 
restore 
...
collapse (mean) dist_15 dist_18 dist_20 dist_30 dist_37 dist_40, by(decile)
save "$results/SEN2021_SWIFTPMT_undercoverage_rural_large.dta", replace 
restore							


* PMT Non-Coverage Decile Graph	
use "$results/SEN2021_SWIFTPMT_undercoverage_rural_large.dta", clear 
sort decile
...
graph twoway   /// 		choosing poorest 20% as true poor population (dist_20)
	...
	legend(off) ytitle("Percent of All Covered Households Rural", size(medsmall)) xtitle("Decile of True Consumption Level (true poor bottom 20%)", size(medsmall)) xlabel(1(1)10, labsize(small)) ylabel(84(3)100,labsize(small))
gr save "$results/SEN2021_undercoverage_truepoor20_rural_large.gph",  replace
...
graph twoway   /// 		choosing poorest 37% as true poor population (dist_37)
	...
	legend(off) ytitle("Percent of All Covered Households Rural", size(medsmall)) xtitle("Decile of True Consumption Level (true poor bottom 37%)", size(medsmall)) xlabel(1(1)10, labsize(small)) ylabel(84(3)100,labsize(small))
gr save "$results/SEN2021_undercoverage_truepoor37_rural_large.gph",  replace


* PMT Coverage Decile Graph									
use "$results/SEN2021_SWIFTPMT_leakage_rural_large.dta", clear 
sort decile
graph twoway   /// 		choosing poorest 20% as true poor population (dist_20)
    ...
    (bar dist_20 decile if decile >= 6,  fcolor(maroon*0.6) lcolor(maroon*0.6)), ///
	 legend(off)ytitle("Percent of All Covered Households", size(medsmall)) xtitle("Decile of True Consumption Level (true poor bottom 20%)", size(medsmall)) xlabel(1(1)10, labsize(small)) ylabel(,labsize(small))  title("Distribution of eligible population Rural")
gr save "$results/SEN2021_leakage_truepoor20_rural_large.gph",  replace
...
graph twoway   /// 		choosing poorest 20% as true poor population (dist_37)
    ...
    (bar dist_37 decile if decile >= 6,  fcolor(maroon*0.6) lcolor(maroon*0.6)), ///
	 legend(off)ytitle("Percent of All Covered Households", size(medsmall)) xtitle("Decile of True Consumption Level (true poor bottom 37%)", size(medsmall)) xlabel(1(1)10, labsize(small)) ylabel(,labsize(small))  title("Distribution of eligible population Rural")
gr save "$results/SEN2021_leakage_truepoor37_rural_large.gph",  replace
